...
def test_():
    ...
    with allure.step('step 1: '):
        r = f.get_polygon_by_address_id(tok, pth.incorrect_adress_id)
        print(r)
        r = json.loads(r.text)
        ok = r['ok']
...
"""
Тест-кейс География 3: Получение зоны объекта по некорректному id адреса
    Шаг 0: Получение токена
        Результат: Код 200
    Шаг 1: Запрос зоны по некорректному id адреса
